...
def OpenAccountWindow():
    ...
    E1.pack(pady=2)
    L3=Label(win,text="Name")
    L3.pack(pady=2)
    E2=Entry(win,width=20)
    E2.pack(pady=2)
    L4=Label(win,text="Balance")
    L4.pack(pady=2)
    E3=Entry(win,width=20)
    E3.pack(pady=2)
    B1=Button(win,text="Create Account",width=20,command=lambda: database.create_account_db(conn,E1.get(),E2.get(),E3.get()))
    B1.pack(pady=5)
